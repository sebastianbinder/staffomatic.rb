...
describe Staffomatic::Client::Authorizations do
  ...
  describe ".create_authorization", :vcr do
    context 'without :idempotent => true' do
      it "creates an API authorization" do
        ...
        assert_requested :post, basic_staffomatic_url("/authorizations")
      end
    ...
    context 'with :idempotent => true' do
      it "creates a new authorization with options" do
        authorization = @client.create_authorization \
          ...
          :scopes => %w(gist)
        expect(authorization.scopes).to be_kind_of Array
        assert_requested :put, basic_staffomatic_url("/authorizations/clients/#{test_staffomatic_client_id}")
      end
  ...
  describe ".authorizations", :vcr do
    it "lists existing authorizations" do
      ...
      assert_requested :get, basic_staffomatic_url("/authorizations")
    end
  end # .authorizations
  ...
  describe ".authorization", :vcr do
    it "returns a single authorization" do
      ...
      assert_requested :get, basic_staffomatic_url("/authorizations/#{authorization.id}")
    end
  end # .authorization
  ...
  describe ".update_authorization", :vcr do
    it "updates and existing authorization" do
      ...
      assert_requested :patch, basic_staffomatic_url("/authorizations/#{authorization.id}")
    end
  ...
  describe ".scopes", :vcr do
    it "checks the scopes on the current token" do
      authorization = @client.create_authorization
      token_client = Staffomatic::Client.new(:access_token => authorization.token)
    ...
    it "checks the scopes on a one-off token" do
      authorization = @client.create_authorization
      Staffomatic.reset!
      expect(Staffomatic.scopes(authorization.token)).to be_kind_of Array
      assert_requested :get, staffomatic_url("/user")
    end
  end # .scopes
  ...
  describe ".delete_authorization", :vcr do
    it "deletes an existing authorization" do
      ...
      VCR.use_cassette 'delete_authorization' do
        ...
        assert_requested :delete, basic_staffomatic_url("/authorizations/#{authorization.id}")
      end
    end
  ...
  describe ".authorize_url" do
    context "with preconfigured client credentials" do
      it "returns the authorize_url" do
        ...
        url = Staffomatic.authorize_url
        expect(url).to eq('https://staffomatic.com/login/oauth/authorize?client_id=id_here')
      end
    end

    context "with passed client credentials" do
      it "returns the authorize_url" do
        url = Staffomatic.authorize_url('id_here')
        expect(url).to eq('https://staffomatic.com/login/oauth/authorize?client_id=id_here')
      end
    end
  ...
  describe ".check_application_authorization", :vcr do
    it "checks an application authorization" do
      ...
      path = "/applications/#{test_staffomatic_client_id}/tokens/#{authorization.token}"
      url = basic_staffomatic_url path,
        :login => test_staffomatic_client_id, :password => test_staffomatic_client_secret
      assert_requested :get, url
      expect(token.user.login).to eq(test_staffomatic_login)
    end
  ...
  describe ".reset_application_authorization", :vcr do
    it "resets a token" do
      ...
      reset_url = basic_staffomatic_url path,
        :login => test_staffomatic_client_id, :password => test_staffomatic_client_secret
  ...
  describe ".revoke_application_authorization", :vcr do
    it "deletes an application authorization" do
      ...
      path = "/applications/#{test_staffomatic_client_id}/tokens/#{authorization.token}"
      revoke_url = basic_staffomatic_url path,
        :login => test_staffomatic_client_id, :password => test_staffomatic_client_secret
  ...
  describe ".revoke_all_application_authorizations" do
    it "deletes all authorizations for an application" do
      path = "/applications/#{test_staffomatic_client_id}/tokens"
      revoke_url = basic_staffomatic_url path,
        :login => test_staffomatic_client_id, :password => test_staffomatic_client_secret
